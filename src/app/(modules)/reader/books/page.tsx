...
export default function BookSearch() {
  ...
  const fetchData = async () => {
    const searchTerm = paramsRouter.get("searchTerm") ?? "";
    let categories = paramsRouter.get("categories");
    let categoriesIds: any[] = [];
    if (categories) {
      categoriesIds =
        paramsRouter.get("categories")?.split(",").map(Number) ?? [];
    }

    setIsLoading(true);
    if (!searchTerm && categoriesIds.length === 0) {
      handleShowToast(
    ...
    <div className="shadow-2xl p-4 rounded-lg" ref={componentRef}>
      ...
      <div className="flex flex-wrap justify-between mb-5">
        <h1 className="font-custom m-0 relative text-2xl text-primary-500 font-bold lg:before:content-[''] lg:before:block md:before:absolute lg:before:absolute xl:before:absolute before:h-full before:w-1 before:bg-primary-500 before:left-0">
          <span className="xl:ps-2 lg:ps-2 md:ps-2">Búsqueda</span>
        </h1>
      </div>
      <div className="flex flex-wrap justify-between">
        <div>
          <span>
            {totalBooks} {totalBooks > 1 ? "libros hallados" : "libro hallado"}{" "}
          </span>
        </div>
        <div className="flex items-center flex-wrap">
          ...
          <button
            className={`flex items-center mr-2 ${
              orderBy === "bookName_asc"
                ? "text-blue-500"
                : orderBy === "bookName_desc"
                ? "text-blue-500"
                : "text-gray-500 hover:text-blue-500"
            }`}
            onClick={() =>
              setOrderBy(
                orderBy === "bookName_asc" ? "bookName_desc" : "bookName_asc"
              )
            }
          >
