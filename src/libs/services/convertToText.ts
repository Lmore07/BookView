...
export async function convertToText(
    ...
    const files = [
        await uploadToGemini(path, mimeType, name),
    ];
...
export async function uploadToGemini(path: string, mimeType: string, name: string) {
    const fileManager = new GoogleAIFileManager(`${process.env.API_KEY_GEMINI}`);
    console.log("File manager", await fileManager.listFiles())
    const uploadResult = await fileManager.uploadFile(path, { mimeType, displayName: "audio", name });
